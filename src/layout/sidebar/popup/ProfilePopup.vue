  ...
  <Popup :nameModal="namePopup" @closeModal="closeModal" :buttonSave="true">
    <div class="flex h-full gap-2 p-2">
      ...
      <div class="grid w-full grid-cols-2 gap-2 grid-rows-8">
        <Input :disabled="true" label="nome" class="col-span-1 row-span-1" id="name" :value="user.name"/>
        <Input :disabled="true" label="empresa" class="col-span-1 row-span-1" id="empresa_id" :value="empresa.name"/>
        <Input :disabled="true" label="Modulo Padrão" class="col-span-2 row-span-1" id="moduledefault_id" :value="module.name"/>
      </div>
    </div>
  </Popup>
</template>
